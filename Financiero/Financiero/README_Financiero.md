...
Clean the data and document the treatment of missing values and data types (614 → 592 records).
Describe the profile of approved vs. rejected applications (69.4% vs. 30.6%).
Measure the relationship between each variable (credit history, dependents, co-applicant, property area, income) and approval.
Build and test a new variable not present in the original dataset: the Loan-to-Income Ratio = Loan Amount ÷ Total Income (applicant + co-applicant).
Compare three models (Logistic Regression, Random Forest, XGBoost) and tune the decision threshold.
Identify the most important variables and translate them into useful credit-risk criteria.
Main findings
...
Dataset originally obtained from Kaggle and edited for this project.
...
Limpiar los datos y documentar el tratamiento de nulos y tipos de datos (614 → 592 registros).
Describir el perfil de las solicitudes aprobadas frente a las rechazadas (69.4% vs. 30.6%).
Medir la relación de cada variable (historial crediticio, dependientes, co-solicitante, área de la propiedad, ingresos) con la aprobación.
Construir y evaluar una variable que no estaba en el dataset original: el Ratio Préstamo/Ingreso = Monto del préstamo ÷ Ingreso total (solicitante + co-solicitante).
Comparar tres modelos (Regresión Logística, Random Forest, XGBoost) y ajustar el umbral de decisión.
Identificar las variables más importantes y traducirlas en criterios útiles para el riesgo crediticio.
...
Dataset obtenido originalmente de Kaggle y editado para este proyecto.
